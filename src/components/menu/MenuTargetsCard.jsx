...
export default function MenuTargetsCard({
  ...
  const recalculateMacroDistribution = () => {
    ...
    const updatedMeals = menuData.meals.map(meal => {
      ...
      return {
        ...meal,
        mealCalories: Math.round(menuData.dailyTotalCalories * ratio),
  ...
  const handleMacroDistributionChange = (macro, percentage) => {
    ...
    const proteinGrams = Math.round((menuData.dailyTotalCalories * (proteinPct / 100)) / 4);
    const fatGrams = Math.round((menuData.dailyTotalCalories * (fatPct / 100)) / 9);
    const carbsGrams = Math.round((menuData.dailyTotalCalories * (carbsPct / 100)) / 4);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="dailyCalories"
            type="number"
            value={menuData.dailyTotalCalories}
            onChange={(e) => {
              onUpdateMenuData({
                ...menuData,
                dailyTotalCalories: parseInt(e.target.value) || 0
              });
