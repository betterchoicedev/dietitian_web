...
export default function Clients() {
  ...
  // Handle URL parameter to auto-open edit dialog for a specific client
  useEffect(() => {
    const editUserCode = searchParams.get('edit');
    
    if (editUserCode && clients.length > 0 && !dialogOpen && !loading) {
      const clientToEdit = clients.find(client => client.user_code === editUserCode);
      
      if (clientToEdit) {
        console.log('🎯 Auto-opening edit dialog for client:', editUserCode);
  ...
  }, [clients, searchParams, dialogOpen, loading]);
