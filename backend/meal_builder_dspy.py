...
class MealBuilderChain(dspy.Module):
    ...
    def _claude_nutrition_fallback(self, ingredient_query: str) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"🔍 Using Claude web search for '{ingredient_query}'...")
...
The data must be for "{ingredient_query}" specifically, not a similar or substitute ingredient."""
            ...
            if all(isinstance(data.get(k), (int, float)) for k in ["calories", "protein_g", "fat_g", "carbohydrates_g"]):
                ...
                logger.info(f"✅ Claude found nutrition for '{ingredient_query}': {data}")
                return {
                    "calories": float(data["calories"]),
...
"""
        ...
        for ingredient in ingredients_list:
            ...
            if nutrition_100g is None:
                logger.warning(f"⚠️ Gemini failed for '{ingredient}' - trying Claude with web search")
...
3. Return JSON: {{"ingredient": {{"portionSI(gram)": X, "household_measure": "text", "brand of pruduct": "brand"}}}}"""
            ...
            logger.info(f"✅ Stage 3 complete: Portions calculated for all ingredients")
            ...
            if cals_ok and protein_ok and fat_ok and carbs_ok:
                logger.info(f"✅ Macro targets achieved! Calories: {total_calories:.1f}/{target_cals}, Protein: {total_protein:.1f}/{target_protein}g, Fat: {total_fat:.1f}/{target_fat}g, Carbs: {total_carbs:.1f}/{target_carbs}g")
                break
            else:
                # If this is the last attempt, let Claude (Stage 4) handle the adjustment
                if refinement_attempt == max_refinement_attempts - 1:
                    logger.warning(f"⚠️ Portions still off after {refinement_attempt + 1} attempts. Letting Claude macro agent adjust portions directly.")
                ...
                logger.warning(f"⚠️ Macro targets not met on attempt {refinement_attempt + 1}: {feedback_parts}")
        
        # ======================================================================
        # STAGE 4: Assemble final meal JSON with validation (Claude + CoT)
        # ======================================================================
        logger.info(f"🧠 Stage 4: Assembling and validating final meal JSON")
...
"""
        ...
        logger.info(f"🤔 CoT Reasoning: {assembly_result.reasoning}")
        ...
        if not (cals_ok and protein_ok and fat_ok and carbs_ok):
            logger.warning(f"⚠️ Claude's final meal doesn't meet targets. Applying programmatic correction...")
            ...
            for correction_iter in range(max_correction_iterations):
                ...
                logger.info(f"📊 Correction iteration {correction_iter + 1}: Macro errors: {macro_errors}")
                    ...
                    if total_value > 0 and ing_value > 0:
                        ...
                        if contribution > 0.1:
                            ...
                            if abs(conservative_scale - 1.0) > 0.01:  # Only log if significant change
                                logger.info(f"   {ing.get('item')}: {old_portion}g → {new_portion}g (factor: {conservative_scale:.3f}, fixing {priority_macro})")
            ...
            logger.info(f"✅ Final corrected totals: {corrected_totals}")
        else:
            logger.info(f"✅ Final meal meets all macro targets: {actual_totals}")
        
        logger.info(f"✅ Stage 4 complete: Final meal JSON assembled and validated")
